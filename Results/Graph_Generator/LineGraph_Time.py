...
def AirDataProcessing(FileName):
    data=ReadCSV(FileName)
    print(data)
    df = DataFrame (data,columns=['Threshold','Epoch','Accuracy','Time'])
    print (df)
    unique_threshold=df.Threshold.unique()
...
def GetDataForApp(app_name):
    if(app_name==1):
        ...
        return AirDataProcessing("../CSV/Air-Pollution-App/Execution_Time_Air_Pollution.csv")
    elif(app_name==3):
