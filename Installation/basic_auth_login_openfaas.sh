...
ip=$(docker info |grep "Node Address: " | tr --delete "Node Address: ")
echo "$ip"
printf "echo -n ""$secret"" | faas-cli login --username=admin --password-stdin --gateway http://""$ip"":8080" > "command.txt"
python LoginAuth.py
